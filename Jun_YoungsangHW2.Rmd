...
A graduated symbol map showing data within a 0.5-mile buffer around each
transit station provides useful information into the trends of
population and median rent near transit stations. To do this, I
calculated the population and median rent density per tract, which were
derived from ACS data. Second, I iterated to intersect over the tracts
with transit station buffer circles, and to multiple by population and
median rent density and area of the cut tracts. Finally, I summed them
...
for(i in 1:nrow(stopBuffer)) {
  stop_intersect <- st_intersection(stopBuffer[i,], allTracts_Dens22) %>% 
    ...
    mutate(partPop = as.numeric(partArea*TotalPop/2034900)) %>% 
...
This graduated symbol map shows that population within 0.5 mile of each
transit station. Central Boston has upto 80,000 people.
...
ggplot() + 
  ...
  labs(title="Population within 0.5 mile of Each Transit Stn", 
       ...
       caption="Data: U.S. Census Bureau, MBTA") +
  guides(color = guide_legend("as.numeric(partVal_pop)"), size = guide_legend("as.numeric(partVal_pop)")) +
  theme_void()+
  theme(legend.position = "right")
```

### 5.2. Map of Median Rent per square mile

annotate each visualization appropriately: The station of highest median
rent is Massachusetts Avenue Station as well, whose value is \$45,000.
...
ggplot() + 
  ...
  labs(title="Median Rent within 0.5 mile of Each Transit Stn", 
       subtitle="Boston (Suffolk County), MA", 
       caption="Data: U.S. Census Bureau, MBTA") +
  guides(color = guide_legend("as.numeric(partVal_med)"), size = guide_legend("as.numeric(partVal_med)")) +
  theme_void()+
  theme(legend.position = "right")
```
